...
void brisiIzSeznama(Stevila* &head, int iskani){
    ...
    if(current==NULL){ // ce iskanega stevila ni v seznamu
        return;
    }
    if(current->next==NULL && current->prev!=NULL){ // ko brisemo na koncu
        current->prev->next=NULL;
        current->prev=NULL;
    }
    else if(current->prev==NULL && current->next!=NULL){ // ko brisemo na zacetku
        current->next->prev=NULL;
        head=current->next;
        current->next=NULL;
    }
    else if(current->next==NULL && current->prev==NULL){ //  ko je seznam prazen
        head=NULL;
    }
    else{ // brisanje vmes
...
void hitrost(Stevila* &head){
    ...
    if(kamVstavi==1){
        ...
        long long int polje_vsota=0;
        start = clock();
        for(int i=0; i<len; i++){
            polje_vsota=polje_vsota+arr[i];
        }
        finish = clock();
        double vsotaPolja = (double)(finish - start) / CLOCKS_PER_SEC;
        cout << "Cas sestevanja  " << len << " elementov v polju " << vsotaPolja << endl;
        cout<<"Vsota je: "<<polje_vsota<<endl;
    }
    else
    ...
    do{
		...
		switch (izbira) {
		    case '1':
                ...
                cin>>iskani;
                current=iskanjeStevila(head, iskani);
                if(current!=NULL)
